ActiveAdmin.register QueenWork do
	...
	after_update do |queen_work|
		...
		if queen_work.queen
			message_str = "<strong>项目<a href='#{queen_work_path(queen_work)}'>#{queen_work.title}</a>, 已经指派给蚁后:<a href='#{queen_path(queen_work.queen_id)}'>#{queen_work.queen.name}</a> </strong>"
