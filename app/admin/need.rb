ActiveAdmin.register Need do
	...
	form do |f|
		...
		f.inputs 'more' do
			...
			f.input	:description
      f.input :reference_product_ids, input_html: {value: f.object.reference_product_ids.split(',').map{|q| Product.find_by_id(q.to_i).null? ? '' : Product.find_by_id(q.to_i).title}}
      f.input :reference_queen_ids, input_html: {value: f.object.reference_queen_ids.split(',').map{|q| User.find_by_id(q.to_i).null? ? '' : User.find_by_id(q.to_i).name}}
